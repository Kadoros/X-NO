...
def build_S_matrix(degree: int):
    ...
    # [질문자님 말씀이 맞았습니다!] 이중 적분 시 스케일 소멸을 막기 위해 반드시 정규화해야 합니다.
...
# ════════════════════════════════════════════════════════════════════
#  CONV LAYERS: 2nd-Order Heinn-X (이중 적분 물리 엔진)
# ════════════════════════════════════════════════════════════════════


def chebyshev_eval_matrix(N: int, degree: int, device=None):
    """임의의 차수에 대한 합성 행렬(T) 생성기"""
    x = torch.linspace(0, 1, N, dtype=torch.float32)
    xn = (2.0 * x - 1.0).clamp(-1 + 1e-6, 1 - 1e-6)
    T_eval = torch.cos(
        torch.acos(xn).unsqueeze(1)
...
class HeinnXConv1D(nn.Module):
    def __init__(self, in_ch, out_ch, degree):
        super().__init__()
        self.degree = degree
        # Path 0: 디테일 및 경계조건(Boundary Condition) 보정
        ...
        # Path 2: 열전도(Heat Equation)의 본질인 2차 이중 적분
        self.W2 = nn.Parameter(
            torch.randn(in_ch, out_ch, degree + 3) / math.sqrt(in_ch * (degree + 3))
        )

        self.register_buffer("S1", build_S_matrix(degree))
        self.register_buffer("S2", build_S_matrix(degree + 1))
        self._cache = {}
    ...
    def forward(self, x):
        ...
        if key not in self._cache:
            T_pinv, T0 = chebyshev_matrices(N, d_eff, x.device)
            T1 = chebyshev_eval_matrix(N, d_eff + 1, x.device)
            T2 = chebyshev_eval_matrix(N, d_eff + 2, x.device)
            self._cache[key] = (T_pinv, T0, T1, T2)

        T_pinv, T0, T1, T2 = self._cache[key]

        c = torch.einsum("bcn,kn->bck", x, T_pinv)

        # Path 0
        m0 = torch.einsum("bck,cok->bok", c, self.W0[:, :, : d_eff + 1])
        out0 = torch.einsum("bok,nk->bon", m0, T0)

        # Path 1
        c_int1 = torch.einsum("bck,lk->bcl", c, self.S1[: d_eff + 2, : d_eff + 1])
        m1 = torch.einsum("bcl,col->bol", c_int1, self.W1[:, :, : d_eff + 2])
        out1 = torch.einsum("bol,nl->bon", m1, T1)

        # Path 2 (이중 적분)
        c_int2 = torch.einsum("bcl,ml->bcm", c_int1, self.S2[: d_eff + 3, : d_eff + 2])
        m2 = torch.einsum("bcm,com->bom", c_int2, self.W2[:, :, : d_eff + 3])
        out2 = torch.einsum("bom,nm->bon", m2, T2)
